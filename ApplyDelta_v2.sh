...
echo "🔧 Applying SumoGame delta (v2)…"

PROJECT_ROOT="$(pwd)"
fail() { echo "❌ $*"; exit 1; }

# Safe in-place replace using Perl with \Q...\E quoting and '@' delimiter
replace() {
  local file="$1"
  local search="$2"
  local replace_str="$3"
  ...
  cp "$file" "$file.bak"
  perl -0777 -pe 's@\Q'"$search"'\E@'"$replace_str"'@g' "$file.bak" > "$file"
  echo "✔ Patched: $file"
}
...
if [[ -f "$GTD" ]]; then
  ...
  # idempotent rel-path normalization
  replace "$GTD" "../engine/time" "../engine/time"
  replace "$GTD" "../engine/store" "../engine/store"
else
